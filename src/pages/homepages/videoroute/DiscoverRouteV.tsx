...
const DiscoverRouteV = () => {
  ...
  useEffect(() => {
    const getRssFeeds = async () => {
      try {
        ...
        const cachedData = sessionStorage.getItem('rssVideosDataDiscover');
        if (cachedData) {
          ...
          sessionStorage.setItem('rssVideosDataDiscover', JSON.stringify(response.data));
  ...
  return (
    <div className='flex mt-5 gap-y-7 w-full flex-wrap flex-row justify-between max-h-[550px] pr-3 pb-5 overflow-y-auto'>
        <div className='flex gap-y-7 w-full flex-wrap lg:flex-row lg:justify-between'>
      {!videos ? (<Loader />) : (videos.map((video: any) => (
        <VideoCard
          key={video.title}
