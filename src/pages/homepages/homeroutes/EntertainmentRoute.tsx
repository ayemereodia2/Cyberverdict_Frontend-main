...
const MyfeedsRoute = () => {
      ...
      try {
        ...
        if (cachedData) {
          ...
          const response = await axios.get(
            "https://cyberverdictbackend.onrender.com/api/feeds/relatedVideos/entertainment?page=1&limit=15",
            config
          );

          // Set the fetched data in localStorage
          sessionStorage.setItem(
            "rssVideosDataEntertainment",
  ...
  useEffect(() => {
    const getRssFeeds = async () => {
      try {
        // Check if data is available in localStorage
        const cachedData = sessionStorage.getItem("rssPostsDataEntertainment");
        if (cachedData) {
          ...
          const response = await axios.get(
            "https://cyberverdictbackend.onrender.com/api/feeds/relatedPosts/entertainment?page=1&limit=15",
            config
          );

          // Set the fetched data in localStorage
          sessionStorage.setItem(
            "rssPostsDataEntertainment",
            JSON.stringify(response.data)
          );
  ...
  useEffect(() => {
    const getSiteDetails = async () => {
      try {
        const cachedData = sessionStorage.getItem("rssWebsitesDataEntertainment");
        if (cachedData) {
          ...
          const response = await axios.get(
            "https://cyberverdictbackend.onrender.com/api/feeds/getSites/entertainment",
            config
          );

          // Set the fetched data in localStorage
          sessionStorage.setItem(
            "rssWebsitesDataEntertainment",
            JSON.stringify(response.data)
          );
