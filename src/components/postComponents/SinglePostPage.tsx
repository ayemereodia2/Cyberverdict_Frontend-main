// import postImg from "./postAssets/media2.png";
// import { PiBookmarks, PiShareNetworkFill } from "react-icons/pi";
...
const SinglePostPage = () => {
  ...
  useLayoutEffect(() => {
    const getRssFeeds = async () => {
      try {
        const cachedData = sessionStorage.getItem(`post${title}`);
        if (cachedData) {
          ...
          const response = await axios.get(
            `https://cyberverdictbackend.onrender.com/api/feeds/post/${title}`,
            config
          );
          sessionStorage.setItem(`post${title}`, JSON.stringify(response.data));
  ...
  useEffect(() => {
    const getRelatedPosts = async () => {
      try {
        const cachedData = sessionStorage.getItem(`relatedPosts${title}`);
        if (cachedData) {
          setRelatedPosts(JSON.parse(cachedData));
        } else {
          const config = {
            headers: {
              "Content-type": "application/json",
          ...
          const response = await axios.get(
            `https://cyberverdictbackend.onrender.com/api/feeds/relatedPosts/${post.category}`,
            config
          );
  ...
  useEffect(() => {
    if (post){
      const checkPostSaved = async () => {
        try {
          const response = await axios.get(
            "https://cyberverdictbackend.onrender.com/api/feeds/checkSavedPost",
            {
              params: {
                title: post.title,
                userId: userInfo?._id,
          ...
          if (response.data.message === "saved post") {
            setSaved(true);
          }
          console.log(saved);
        } catch (error) {
  ...
  const savePost = async () => {
    try {
      ...
      const response = await axios.post(
        "https://cyberverdictbackend.onrender.com/api/feeds/savePost",
        {
          title: post.title,
          userId: userInfo?._id,
        },
      ...
      notifier.show(response.data.message,"Success","success");
      if (response.data.message === "Post saved successfully") {
        setSaved(true);
      } else if (response.data.message === "Post unsaved successfully") {
        setSaved(false);
      }
      console.log(saved);
    } catch (error: any) {
  ...
  return !post ? (
    ...
    <div className="bg-homeBg lg:pl-56 py-7 mt-5">
      <Notifier/>
      <div className="bg-white p-4 lg:p-7 w-full lg:w-3/4">
        ...
        <div className="flex flex-row gap-6 mt-5 items-center">
          <p className="tracking-wide text-sm">
            <span className="text-[#A09D9D]">
              {calculateTimeAgo(post.pubDate)} -{" "}
            </span>
            <span className="text-primaryColor">
              {calculateReadingTime(post.contentSnippet)}
            </span>
          </p>
        ...
        <div className="text-[#A09D9D] text-sm leading-6 tracking-wide text-left mt-3">
          {post.contentSnippet}
          <a href={post.link} className="text-blue-700">
            {" "}
            Read more...
          </a>
        </div>
        ...
        <div className="mt-10">
          <p className="text-left font-semibold leading-6">Related posts</p>
          <div className="mt-5 w-full flex flex-row gap-7 flex-wrap">
            {!relatedPosts ? (
              <Loader />
            ) : (
              relatedPosts
                ...
                .map((relatedPost: any) => (
                  <PostCard
                    ...
                    pubDate={relatedPost.pubDate}
                    contentSnippet={relatedPost.contentSnippet}
                  />
                ))
            )}
          </div>
