  ...
  useEffect(() => {
    const getRssFeeds = async () => {
      try {
        ...
        const cachedData = sessionStorage.getItem(`posts${title}`);
        if (cachedData) {
          ...
          const response = await axios.get(
            `https://cyberverdictbackend.onrender.com/api/feeds/getSitePosts/${title}`,
            config
          );

          // Set the fetched data in localStorage
          sessionStorage.setItem(`posts${title}`, JSON.stringify(response.data));
